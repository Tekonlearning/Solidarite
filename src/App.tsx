      ...
      {showLoginModal && (
        ...
        >
          <div 
            ...
            className="bg-[#FBF9F7] rounded-3xl max-w-md w-full shadow-2xl border border-slate-200/80 overflow-hidden font-sans p-6 text-slate-800 animate-in zoom-in-95 duration-200 relative animate-duration-150"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setShowLoginModal(false)}
              className="absolute top-4 right-4 bg-white/80 hover:bg-slate-105 p-1.5 rounded-full border border-slate-200 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer"
            >
            ...
            <div className="text-center mb-6 mt-2">
              <span className="w-12 h-12 bg-orange-100 text-orange-600 rounded-2xl flex items-center justify-center mx-auto mb-3 font-black text-xl">
                Sòl
              </span>
              <h2 className="text-lg font-black tracking-tight text-slate-900 uppercase">
                {language === 'creole' ? 'Konekte sou SaNa' : 'Connexion à SaNa'}
              </h2>
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                {language === 'creole' 
                  ? 'Konekte kòm yon itilizatè reyèl ak Google, oswa sèvi ak similatè tès la!' 
                  : "Connectez-vous en tant qu'utilisateur réel avec Google, ou utilisez le simulateur de test !"}
              </p>
            </div>
            ...
            <div className="space-y-4">
              {/* AUTH ERROR DISPLAY */}
              {authError && (
                <div className="bg-rose-50 border border-rose-250 p-3.5 rounded-xl text-xs text-rose-950 flex flex-col space-y-2">
              ...
              {/* GOOGLE AUTH WITH REAL API CALLS */}
              <div>
                <span className="text-[9px] font-bold text-slate-400 tracking-widest uppercase block mb-2">
                  {language === 'creole' ? 'RELE AKSÈ SEKIRIZE' : 'ACCÈS SÉCURISÉ RÉEL'}
                </span>
                <button
                  type="button"
                  onClick={async () => {
                    try {
                      ...
                      console.error(err);
                      alert(language === 'creole' 
                        ? 'Koneksyon Google la pa reyisi. Tanpri verifye si w sou yon nouvo onglet devan.' 
                        : "L'authentification Google a échoué. Veuillez essayer dans un nouvel onglet.");
                    }
                  }}
                  className="w-full flex items-center justify-center space-x-3 bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 font-bold py-3 px-4 rounded-xl shadow-xs transition-colors cursor-pointer active:scale-98"
                >
                  ...
                  <span className="text-xs font-black tracking-wide uppercase">
                    {language === 'creole' ? 'Konekte ak Google' : 'Connexion Google'}
                  </span>
                </button>
              </div>
            </div>
